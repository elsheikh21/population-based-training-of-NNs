...
class GAN(object):
    ...
    def __init__(self, worker_idx=-1, batch_size=64, z_dim=128, epochs=100, data_X=None, data_y=None):
        ...
        self.checkpoint_dir = os.path.join(
            os.getcwd(), 'pbt_gans', 'checkpoint')
        self.log_dir = os.path.join(os.getcwd(), 'pbt_gans', 'logs')
        self.image_dir = os.path.join(
            os.getcwd(), 'pbt_gans', 'images/{}'.format(worker_idx))
        ...
        # train
        self.learning_rate_D = tf.get_variable(
            'learning_rate_D', initializer=tf.constant(2e-4))
        self.learning_rate_G = tf.get_variable(
            'learning_rate_G', initializer=tf.constant(2e-4))
...
#        self.data_X = np.reshape(self.data_X, [-1, 32*32*3])
        ...
        DATA_DIR = os.path.join(os.getcwd(), 'pbt_gans', 'data')
        self.train_gen, self.dev_gen = lib.cifar10.load(
            self.batch_size, data_dir=DATA_DIR)
...
#        # load pretrained inception network (code from tensorflow / openAI)
    ...
    def Discriminator(self, inputs):
        ...
        output = self.Conv2D('Discriminator.1', 3,
                             self.z_dim, 5, output, stride=2)
        output = LeakyReLU(output)
        print('D: {}'.format(output.get_shape()))

        output = self.Conv2D('Discriminator.2', self.z_dim,
                             2*self.z_dim, 5, output, stride=2)
        output = self.Batchnorm('Discriminator.BN2', [0, 2, 3], output)
        print('D: {}'.format(output.get_shape()))
        output = LeakyReLU(output)
        print('D: {}'.format(output.get_shape()))

        output = self.Conv2D('Discriminator.3', 2*self.z_dim,
                             4*self.z_dim, 5, output, stride=2)
        ...
        output = self.Linear('Discriminator.Output', 4 *
                             4*4*self.z_dim, 1, output)
        print('D: {}'.format(output.get_shape()))
    ...
    def Generator(self, n_samples, noise=None):
        ...
        output = self.Linear('Generator.Input', 128, 4 *
                             4*4*self.z_dim, self.noise)
        output = self.Batchnorm('Generator.BN1', [0], output)
        output = tf.nn.relu(output)
        output = tf.reshape(output, [-1, 4*self.z_dim, 4, 4])

        output = self.Deconv2D(
            'Generator.2', 4*self.z_dim, 2*self.z_dim, 5, output)
        output = self.Batchnorm('Generator.BN2', [0, 2, 3], output)
        output = tf.nn.relu(output)

        output = self.Deconv2D(
            'Generator.3', 2*self.z_dim, self.z_dim, 5, output)
    ...
    def build_model(self):
        ...
        self.inputs = tf.placeholder(
            tf.int32, shape=[self.batch_size, self.output_dim])
        ...
        self.g_optim = tf.train.AdamOptimizer(
            self.learning_rate_G, beta1=0.5).minimize(self.g_loss, var_list=gen_list)
        self.d_optim = tf.train.AdamOptimizer(
            self.learning_rate_D, beta1=0.5).minimize(self.d_loss, var_list=disc_list)

        self.z = tf.constant(np.random.normal(
            size=(self.batch_size, 128)).astype('float32'))
        ...
        d_log_lr_sum = tf.summary.scalar(
            "d_log_learning_rate", tf.log(self.learning_rate_D))
        g_log_lr_sum = tf.summary.scalar(
            "g_log_learning_rate", tf.log(self.learning_rate_G))

        inception_sum = tf.summary.scalar(
        ...
        self.g_sum = tf.summary.merge(
            [d_loss_fake_sum, g_loss_sum, g_lr_sum, g_log_lr_sum])
        self.d_sum = tf.summary.merge(
            [d_loss_real_sum, d_loss_sum, d_lr_sum, d_log_lr_sum])

        # define explore graph
        coin_flip_D = tf.cast(tf.random_uniform(
            shape=[], minval=0, maxval=1+1, dtype=tf.int32), tf.float32)
        coin_flip_G = tf.cast(tf.random_uniform(
            shape=[], minval=0, maxval=1+1, dtype=tf.int32), tf.float32)

        self.explore_learning_D = tf.assign(
            self.learning_rate_D, 2*self.learning_rate_D*coin_flip_D + 0.5*self.learning_rate_D*(1-coin_flip_D))
        self.explore_learning_G = tf.assign(
            self.learning_rate_G, 2*self.learning_rate_G*coin_flip_G + 0.5*self.learning_rate_G*(1-coin_flip_G))

        # update inception tensor
        self.inception_value = tf.placeholder(tf.float32)
        self.update_inception = tf.assign(
            self.inception_score, self.inception_value)
        ...
        for iteration in range(100000):
            ...
            if iteration % 100 == 99:
                ...
                for images, _ in dev_gen():
                    _dev_disc_cost = self.mon_sess.run(self.disc_cost, feed_dict={
                                                       self.real_data_int: images})
    ...
    def step(self, idx, epoch, verbose=True):
        ...
        # Train generator
        _gen_cost, _, summary_str = self.mon_sess.run(
            [self.gen_cost, self.gen_train_op, self.g_sum])
        ...
        if strategy == "TS":
            ...
            if 'current' in bottom_20:
                ...
                if exploit_idx != 'current':
                    do_explore = True
                    print("Worker {} (EXPLOIT): inheriting Worker {}'s weights/hyperparams".format(
                        worker_idx, exploit_idx))
                    self.load(exploit_idx)
            else:
                print(
                    "Worker {} (EXPLOIT): is not in the bottom 20, no action".format(worker_idx))
    ...
    def load(self, worker_idx):
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            self.saver.restore(
                self.mon_sess, os.path.join(worker_dir, ckpt_name))
    ...
    def load_saved_session(self):
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            print(
                "Successfully loaded checkpoint from epoch {} idx {}".format(epoch, idx))
        else:
            print("Could not find checkpoint")
        ...
        with tf.Graph().as_default():
            ...
            with tf.Session() as new_sess:
                ...
                for var_name, _ in tf.contrib.framework.list_variables(worker_dir):
                    ...
                    # re.compile('(.*)_w\d+$')
                    worker_regex = re.compile('(.*)_w\d+(.*)')
                    m = re.match(worker_regex, var_name)
                    if m:
                        new_name = m.group(
                            1) + '_w{}'.format(self.worker_idx) + m.group(2)
                    else:
                        new_name = var_name
    ...
    def get_inception_score(self):
        ...
        return get_inception_score(self, list(all_samples))
